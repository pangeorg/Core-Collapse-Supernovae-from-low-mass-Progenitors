...
For the electron-capture model was collapsed with \prom which employs parametrized scheme by \citet{Liebendoerfer2005}. The necessary $\rho-Y_{e}$-trajectory was provided by \cite{Huedepohl2018}.
...
\vertexprom, \NY{or short \vertex,}{} is a finite-volume hydrodynamics code with three-flavor, energy-dependent, ray-by-ray-plus (RbR+) neutrino transport including the full set of neutrino reactions and microphysics as detailed in \cite{Rampp2002}.  The numerical grid employed is the axis-free YinYang-Grid developed by \cite{Kageyama2004} and implemented in \vertexprom by \cite{Melson2015}. It significantly reduces the artifacts of purely spherical grids on the polar axes. 
...
% Nue Transport 
Different to the sophisticated neutrino transport in \vertex, \prom employs a grey transport scheme under the assumption of low neutrino-opacities and calculates the $\nu_{\mathrm{e,x}}$-fluxes by an analytic formula described in detail by \cite{Scheck}\footnote{Some improvements of the neutrino transport described in detail in \cite{Scheck} are give in the Appendix \ref{Appendix:Neutrino}.}. The details of this scheme are described in the next section.
% Grav
...
As already stated by \cite{Janka2007}, the structure of the progenitor is the pivotal element enabling the fast and almost spherical expansion of the forward shock and early rise of explosion energies which are shown in \autoref{fig:eexp}. 
...
\begin{equation}
  ...
  \Big\Sigma_{\mathrm{RT}}(t) = \frac{\xi}{\xi_0}(t) = \mathrm{exp} \left( \int_0^t \sigma_{RT} (t') dt' \right),
\end{equation}
